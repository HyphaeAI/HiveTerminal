...
class VibeConfig(BaseSettings):
    ...
    def _check_api_backend_compatibility(self) -> VibeConfig:
        try:
            ...
            # Check Mistral backend compatibility
            if is_mistral_api and provider.backend != Backend.MISTRAL:
                raise WrongBackendError(provider.backend, is_mistral_api)
            
            # Check that non-Mistral APIs use either GENERIC or LITELLM backend
            if not is_mistral_api and provider.backend not in (Backend.GENERIC, Backend.LITELLM):
                raise WrongBackendError(provider.backend, is_mistral_api)
