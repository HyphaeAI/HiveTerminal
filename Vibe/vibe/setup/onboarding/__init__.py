...
class OnboardingApp(App[str | None]):
    ...
    def on_mount(self) -> None:
        ...
        # API key screen will be installed dynamically with the selected provider
...
def run_onboarding(app: App | None = None) -> None:
    ...
    match result:
        ...
        case "completed":
            rprint(
                '\nSetup complete 🎉. Run "hive" to start using HiveTerminal.\n'
            )
        case "skipped":
            rprint(
                '\nSetup skipped. You can configure your API keys later in the config file.\n'
                'Run "hive" to start using HiveTerminal.\n'
            )
