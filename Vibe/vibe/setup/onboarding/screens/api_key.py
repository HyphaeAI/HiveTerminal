...
PROVIDER_HELP = {
    ...
    "github": ("https://github.com/settings/tokens", "GitHub Settings"),
    "google": ("https://aistudio.google.com/app/apikey", "Google AI Studio"),
...
class ApiKeyScreen(OnboardingScreen):
    ...
    NEXT_SCREEN = "model_selection"

    def __init__(self, provider_name: str | None = None) -> None:
        super().__init__()
        
        # If provider is passed, use it; otherwise get from config
        if provider_name:
            ...
            if provider_name in provider_map:
                name, env_var = provider_map[provider_name]
                # Create a simple provider object
                class SimpleProvider:
                    def __init__(self, name, api_key_env_var):
                        self.name = name
                        self.api_key_env_var = api_key_env_var
                self.provider = SimpleProvider(name, env_var)
            else:
    ...
    def _save_and_finish(self, api_key: str) -> None:
        ...
        # Install model selection screen with the provider
        from vibe.setup.onboarding.screens.model_selection import ModelSelectionScreen
        self.app.install_screen(
            ModelSelectionScreen(provider_name=self.provider_name),
            "model_selection"
        )
